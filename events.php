...
if (isset($_POST['add'])) {
    ...
    $conn->query("INSERT INTO Events (evCode, evName, evDate, evVenue, evRFee)
                  VALUES ('$evCode', '$evName', '$evDate', '$evVenue', '$evFee')");
}
...
if ($search != "") {
    $events = $conn->query("SELECT * FROM Events
                            WHERE evCode LIKE '%$search%' 
                               OR evName LIKE '%$search%' 
                               OR evVenue LIKE '%$search%'
                            ORDER BY evDate ASC");
} else {
    $events = $conn->query("SELECT * FROM Events ORDER BY evDate ASC");
}
?>
<!DOCTYPE html>
<html>
...
<h2>Add Event</h2>
<form method="post">
    EVENT CODE: <input type="text" name="evCode" required><br>
...
<table border="1" cellpadding="6">
    ...
    <tr>
        ...
        <td>
            ...
            <a href="?delete=<?= $row['evCode'] ?>" onclick="return confirm('Delete this event?')">Delete</a>
        </td>
    </tr>
    <?php endwhile; ?>
</table>
